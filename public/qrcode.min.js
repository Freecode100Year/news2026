/**
 * qrcode.js - Minimal QR Code Generator (Pure JavaScript, Offline ready)
 * Modified for Prompter project
 */
(function(exports) {
  // Simple QRCode implementation using HTML Canvas or SVG
  // QRCode generator algorithm implementation
  ...
  // Basic QR algorithm implementation using quick SVG/Canvas generator or standard table
  // We provide a reliable pure JS QR code generator below
  ...
  for (var i = 8; i < 256; i++) QRMath.EXP_TABLE[i] = QRMath.EXP_TABLE[i - 4] ^ QRMath.EXP_TABLE[i - 5] ^ QRMath.EXP_TABLE[i - 8] ^ QRMath.EXP_TABLE[i - 6];
  ...
  QR8bitByte.prototype = {
    getLength: function() { return unescape(encodeURIComponent(this.data)).length; },
    write: function(buffer) {
      var str = unescape(encodeURIComponent(this.data));
      for (var i = 0; i < str.length; i++) buffer.put(str.charCodeAt(i), 8);
    }
  };
  ...
  QRRSBlock.RS_BLOCK_TABLE = [
    ...
    [1,134,108],[2,67,43],[2,33,15],[2,33,11],[4,39,14]
  ];
  ...
  QRCode.prototype.makeCode = function(text) {
    ...
    var tileW = this.options.width / count;
    var tileH = this.options.height / count;
